...
public class SampleAPIMapCreator : MonoBehaviour
{
	...
	private ArcGISPoint geographicCoordinates = new(-74.054921, 40.691242, 3000, ArcGISSpatialReference.WGS84());
	...
	private void CreateSkyComponent()
	{
		...
		if (currentSky)
		{
			...
			var skyComponent = currentSky.gameObject.GetComponent<ArcGISSkyRepositionComponent>();
	...
	{
		if (mapComponent.GetComponent<ArcGISViewStateLoggingComponent>() == null)
		{
			mapComponent.gameObject.AddComponent<ArcGISViewStateLoggingComponent>();
		}
	}
	// @@End(ViewState)
	...
	public void CreateArcGISMap()
	{
		...
		{
			// Set the Basemap
			Basemap = new Esri.GameEngine.Map.ArcGISBasemap(Esri.GameEngine.Map.ArcGISBasemapStyle.ArcGISImagery, APIKey),

			// Create the Elevation
			Elevation = new Esri.GameEngine.Map.ArcGISMapElevation(new Esri.GameEngine.Elevation.ArcGISImageElevationSource("https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/Terrain3D/ImageServer", "Terrain 3D", ""))
			{
		...
		if (map.MapType == Esri.GameEngine.Map.ArcGISMapType.Local)
		{
			...
			var extentCenter = new ArcGISPoint(-74.054921, 40.691242, 3000, ArcGISSpatialReference.WGS84());
