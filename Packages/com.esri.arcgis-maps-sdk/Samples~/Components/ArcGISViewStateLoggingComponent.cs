...
namespace Esri.ArcGISMapsSDK.Samples.Components
{
	...
	public class ArcGISViewStateLoggingComponent : MonoBehaviour
	{
		...
		private void SubscribeToViewStateEvents()
		{
			...
			mapComponent.View.ElevationSourceViewStateChanged += (GameEngine.Elevation.Base.ArcGISElevationSource layer, ArcGISElevationSourceViewState elevationSourceViewState) =>
			{
				...
				if ((status.HasFlag(ArcGISElevationSourceViewStatus.Error) || status.HasFlag(ArcGISElevationSourceViewStatus.Warning)) && message != null)
				{
					...
					additionalInfo.TryGetValue("Additional Message", out var additionalMessage);
			...
			mapComponent.View.LayerViewStateChanged += (GameEngine.Layers.Base.ArcGISLayer layer, ArcGISLayerViewState layerViewState) =>
			{
